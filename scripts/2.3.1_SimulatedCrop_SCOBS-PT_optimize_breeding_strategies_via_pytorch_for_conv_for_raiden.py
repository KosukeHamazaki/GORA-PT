...
nGenerationProceed = int(args[1])   # int(args[1])
nIterSimulation = int(args[2])      # int(args[2])
nIterSimulationUnit = 50


selectTypeName = str(args[3])       # str(args[3]): "SI1"; "SI2"; "SI3"; "SI4"
selCriTypeName = str(args[4])       # str(args[4]): "WBV"; "BV+WBV"; "WBV+GVP"; "BV+WBV+GVP"


### Parameter Settings for optimization  
optimMethod = str(args[5])          # str(args[5]): "SGD", "Adam", "Adadelta"
useScheduler = bool(args[6])        # bool(args[6])

learningRate = float(args[7])       # float(args[7])
nIterOptimization = int(args[8])    # int(args[8])
gvpInit = str(args[9])              # str(args[9]): "Mean", "Zero"
betas = [0.1, 0.1]
...
### Read dataset
print("Read dataset")
simDataDict = BSPTR.readSimulatedDataInR(dirPyTorch = dirPyTorch,
                                        nChr = nChr)
...
bsInfoInit = BSPTR.BsInfo(founderHaplo = founderHaplo.to(device),
                         genoMap = genoMap,
...
breederInfoInit = BSPTR.BreederInfo(breederName = "Hamazaki",
                                   bsInfo = bsInfoInit,
                                   mrkPos = mrkPos,
                                   mrkNames = None,
                                   initGenotyping = True,
...
def breedingSimulations(logitHTensor):
    ...
    for repeatNo in range(nRepeat):
        ...
        self = BSPTR.SimBs(bsInfoInit = bsInfoInit,
                           mrkPos = mrkPos,
