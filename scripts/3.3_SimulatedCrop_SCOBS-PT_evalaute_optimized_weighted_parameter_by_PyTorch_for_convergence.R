###################################################################################################################
######  Title: 3.3_SimulatedCrop_SCOBS-PT_evalaute_optimized_weighted_parameter_by_PyTorch_for_convergence   ######
######  Author: Kosuke Hamazaki (hamazaki@ut-biomet.org)                                                     ######
######  Affiliation: Lab. of Biometry and Bioinformatics, The University of Tokyo                            ######
######  Date: 2024/11/10 (Created), 2024/11/11 (Last Updated)                                                ######
###################################################################################################################
  ...
  for (selCriTypeName in selCriTypeNames) {
    ...
    for (gvpInit in gvpInits) {
      ...
      if (file.exists(fileNameOptimInfo)) {
        ...
        nGrid <- 5
        nIterOptNow <- length(optimInfo$logitHTensorList) %/% nGrid * nGrid
        logitHEvalList <- lapply(
          X = optimInfo$logitHTensorList[c(1, seq(nGrid, nIterOptNow, by = nGrid))],
          FUN = np$array
        )
        names(logitHEvalList) <- paste0("NBS", seq(0, nIterOptNow, by = nGrid) * nIterSimulationOpt)
        logitHEvalList <- logitHEvalList[!duplicated(logitHEvalList)]
        nEvalsGORA <- length(logitHEvalList)
        for (evalGORANo in 1:nEvalsGORA) {
          st <- Sys.time()
          evalGORAName  <- names(logitHEvalList)[evalGORANo]
          ...
          fileNameSimBsBVConv <- paste0(dirSimBs, scriptID, "_", simBsNameConv,
    ...
    if (file.exists(fileNameOptimInfoSto)) {
      ...
      for (evalORANo in 1:nEvalsORA) {
        evalORAName <- rownames(hVecMat)[evalORANo]
        print(paste0("ORA: ", evalORAName))
        hVecSto0 <- hVecMat[evalORANo, ]
        ...
        fileNameSimBsStoSOO <- paste0(dirSimBs, scriptID, "_", simBsNameStoSOO,
